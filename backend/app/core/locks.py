...
@asynccontextmanager
async def advisory_lock(lock_name: str, timeout: int = 5, raise_on_fail: bool = False):
    """
    ...
    Args:
        ...
        raise_on_fail (bool): If True, raises RuntimeError on lock timeout.
                              If False (default), logs a warning and skips the body.

    Raises:
        RuntimeError: Only if raise_on_fail=True and lock cannot be acquired.
    """
        ...
        try:
            ...
            if not locked:
                if raise_on_fail:
                    ...
                    logger.warning(
                        f"Skipping '{lock_name}' — lock held by another process after {timeout}s. "
                        f"This stage will retry on next scheduled run."
                    )
